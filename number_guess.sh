...
DISPLAY() {
  ...
  if [[ $USER_ID ]]; then
    #get games played
    GAMES_PLAYED=$($PSQL "select count(user_id) from guessed where user_id = '$USER_ID'")

    #get best game (guess)
    BEST_GUESS=$($PSQL "select min(guess) from guessed where user_id = '$USER_ID'")
...
GAME() {
  ...
  while [[ $GUESSED = 0 ]]; do
    ...
    if [[ ! $GUESS =~ ^[0-9]+$ ]]; then
      ...
      #insert into db
      INSERTED_TO_GAMES=$($PSQL "insert into guessed(user_id, guess) values($USER_ID, $TRIES)")
      GUESSED=1
    #if greater
